...
def fusion(x1, x2, model):
    with torch.no_grad():
        ...
        out_f, _, _, _ = model(x1,x2)
        out = out_f.squeeze(0).detach().cpu()
        # feature2 = model.encoder(x2)
        # feature_fusion = fusion_layer(feature1, feature2)
        # out = model.decoder(feature).squeeze(0).detach().cpu()
...
def test(test_path, model, save_path='./test_result/'):
    img_list = glob(test_path + '*')  # ./test_dataset/ir_vis_set/ir184.png
    img_num = len(img_list) / 2
    suffix = img_list[0].split('.')[-1]   # 获得后缀 png
    img_name_list = list(
        set([img_list[i].split('/')[-1].split('.')[0].strip(string.digits) for i in range(len(img_list))]))  # 'ir' 'vis'

    fusion_phase = test_gray()

    for i in range(int(img_num)):
        img1_path = test_path + img_name_list[0] + str(i) + '.' + suffix   # ./test_dataset/ir_vis_set/ir0.png
